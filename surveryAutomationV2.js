/*
    |--------------------------------------------------------------------------
    | JavaScript Code for Auto-Filling Form
    |--------------------------------------------------------------------------
    |
    | This script automates filling a contribution form by 
    | retrieving team member IDs and scores from local storage or prompting
    | the user to input them if not already stored. The scores are shuffled
    | before being inputted into the form fields.
    |
    | Usage:
    | - Store the JavaScript code in localStorage.
    | - On-page reload, fetch, and execute the stored JavaScript code.
    | 
...
function fillContributionForm() {
  ...
  if (teamMemberIDs.length === 0) {
    for (let i = 1; i <= 7; i++) {
      let id = prompt(\`Enter ID for team member \${i} (Note: Make Your Id the last one):\`);
  ...
  if (scores.length === 0) {
    for (let i = 1; i <= 7; i++) {
      ...
      if (!isNaN(score) && score <= 100) {
        ...
        console.log(\`Invalid score entered for team member \${i}. Using default score.\`);
  ...
  console.log("Sum of scores:", scores.reduce((a, b) => a + b, 0));

  // Get all ID and score input fields
  const idInputs = document.querySelectorAll('input[placeholder="Number must be between 10835819 ~ 22011242"]');
  const scoreInputs = document.querySelectorAll('input[placeholder="Please enter a number less than or equal to 100"]');
  ...
  for (let i = 0; i < Math.min(teamMemberIDs.length, scores.length, 7); i++) {
    if (idInputs[i] && scoreInputs[i]) {
      ...
      console.log(\`Set ID \${i + 1} to \${teamMemberIDs[i]} and score to \${scores[i]}\`);
    } else {
...
// Store the JavaScript code in localStorage
localStorage.setItem('storedJSCode', jsCode);



// Fetch the JavaScript code from localStorage
const storedJSCode = localStorage.getItem('storedJSCode');
...
if (storedJSCode) {
  ...
  console.log("No JavaScript code found in localStorage.");
}
